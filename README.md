...
### Animated six-game dashboard

![Completed six-game Cops and Robbers dashboard](assets/evidence/gui-full-match.png)

The native Tkinter dashboard animates all six sub-games move by move, including agent movement, progress, scores, winners, and the series tracker. Run the genuine playback with:

```powershell
uv run cops-and-robbers gui --demo
```

The committed image is a reproducible final-frame capture from that playback, generated by `scripts/capture_gui.ps1`; it is not a mockup.

### Reproducible command evidence

| Autonomous six-game match | Local cop/thief MCP smoke |
|---|---|
| ![Headless deterministic match output](assets/evidence/headless-match.png) | ![Local MCP contract output](assets/evidence/mcp-smoke.png) |

### IQL pipeline results

| Cop learning curve | Thief learning curve |
|---|---|
| ![Cop learning curve](results/plots/learning_curve_cop.svg) | ![Thief learning curve](results/plots/learning_curve_thief.svg) |

| Training loss | Fixed-opponent baseline comparison |
|---|---|
