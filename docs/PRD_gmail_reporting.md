...
After all six valid sub-games, generate one canonical JSON report and either write an exact dry-run preview or explicitly send one email to `rmisegal+marl@gmail.com`. Reporting consumes authoritative `MatchResult` objects and never recalculates outcomes.
