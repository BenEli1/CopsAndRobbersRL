...
After exactly six valid sub-games, the system shall generate one schema-valid JSON email body for `rmisegal+marl@gmail.com`. Dry-run shall write `results/report_email_preview.json`; live sending is disabled by default and must never expose credentials.
...
- Student A is Ben Eli (`319086435`). The required group code and any Student B/C identities remain configuration inputs and must be confirmed before live reporting.
